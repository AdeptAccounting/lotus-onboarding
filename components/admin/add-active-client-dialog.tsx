...
import { useCreateActiveClient } from '@/hooks/useClients';
import { SERVICE_TYPE_LABELS, type ServiceType } from '@/types';
import { toast } from 'sonner';
import { UserPlus } from 'lucide-react';

export function AddActiveClientDialog() {
  ...
  const createClient = useCreateActiveClient();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md rounded-2xl border-[#E8D8E0]">
        <DialogHeader>
          <DialogTitle className="text-[#6B3A5E] text-lg">Add Existing Client</DialogTitle>
          <p className="text-xs text-[#8B7080] mt-1">
            Add a client who has already completed onboarding outside this system.
          </p>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label className="text-[#5C4A42] text-sm">Service Type</Label>
              <select
                value={serviceType}
